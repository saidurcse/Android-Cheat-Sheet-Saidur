...
public class MergeOverlappingIntervals {
    ...
    public static List<Tuple<Integer, Integer>> mergeIntervals(List<Tuple<Integer, Integer>> v1) {
        ...
        v2.add(new Tuple<>(v1.get(0).x, v1.get(0).y)); // 10,12

        for(int i=1; i<v1.size(); i++) {
            int x1 = v1.get(i).x; //11
            int y1 = v1.get(i).y; // 15
            int x2 = v2.get(v2.size()-1).getX(); // 10
            int y2 = v2.get(v2.size()-1).getY(); // 12

            if(y2 >= x1) {
                v2.get(v2.size()-1).setY(Math.max(y1,y2)); // 10,15
            } else {
